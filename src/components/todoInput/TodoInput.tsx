...
const TodoInput: FC = () => {
  ...
  const [isInputPositive, setIsInputPositive] = useState<boolean>(true);
  const todoInputRef = useRef<null | HTMLFormElement>(null);

  const handleInputOnClick = function () {
    if (text) {
      dispatch(addTodo(text));
      setIsInputPositive(true);
      setText("");
    } else {
      setIsInputPositive(false);
    }
  };

  const handleInputOnClickOutside = useCallback(
    (e: MouseEvent) => {
      if (isInputPositive) return;
      if (
        todoInputRef.current &&
        !todoInputRef.current.contains(e.target as Node)
      ) {
        setIsInputPositive(true);
      }
    },
  ...
  useEffect(() => {
    document.addEventListener("click", handleInputOnClickOutside);

    return () =>
      document.removeEventListener("click", handleInputOnClickOutside);
  }, [handleInputOnClickOutside]);

  return (
    <Container
      fluid
      className="d-flex align-items-center justify-content-center todoInput"
    >
      <Form
        ref={todoInputRef}
        onSubmit={(e) => {
        ...
        className="d-flex column-gap-4 w-100"
      >
        <Form.Group className="w-50 flex-grow-1" controlId="todoName">
          <Form.Control
            value={text}
            name="todoName"
            onChange={(e) => setText(e.currentTarget.value)}
            placeholder="Поставьте себе любую задачку..."
            className={`todoInput__input p-3 ${
              isInputPositive ? "" : "border border-danger shake-horizontal"
            }`}
          />
        </Form.Group>
        <Button
        ...
        >
          <Container className="d-flex column-gap-2 align-items-center p-2">
            <span>Добавить!</span>
            <svg
              width="16"
              height="16"
